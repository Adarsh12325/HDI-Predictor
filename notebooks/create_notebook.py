import os
import json

# Define the notebook structure as a dictionary
notebook = {
 "cells": [
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "# Exploratory Data Analysis & Model Training\n",
    "\n",
    "This notebook performs Exploratory Data Analysis (EDA) on the synthesized Human Development Index (HDI) dataset and visualizes the relationships between indicators and the HDI score."
   ]
  },
  {
   "cell_type": "code",
   "execution_count": None,
   "metadata": {},
   "outputs": [],
   "source": [
    "import os\n",
    "import numpy as np\n",
    "import pandas as pd\n",
    "import matplotlib.pyplot as plt\n",
    "import seaborn as sns\n",
    "\n",
    "# Set style for plots\n",
    "sns.set_theme(style=\"darkgrid\")\n",
    "plt.rcParams[\"figure.figsize\"] = (10, 6)"
   ]
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "## 1. Load the Dataset"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": None,
   "metadata": {},
   "outputs": [],
   "source": [
    "df = pd.read_csv('../dataset/hdi_dataset.csv')\n",
    "df.head()"
   ]
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "## 2. Basic Dataset Checks"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": None,
   "metadata": {},
   "outputs": [],
   "source": [
    "print(\"Number of unique countries:\")\n",
    "print(df['Country'].nunique())\n",
    "print(\"\\nUnique country list snippet:\")\n",
    "print(df['Country'].unique()[:10])"
   ]
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "## 3. Visualization: Mean Years of Schooling vs HDI Score\n",
    "\n",
    "We use a strip plot (and scatter plot) to see how Mean Years of Schooling relates to the overall HDI score."
   ]
  },
  {
   "cell_type": "code",
   "execution_count": None,
   "metadata": {},
   "outputs": [],
   "source": [
    "plt.figure(figsize=(10, 6))\n",
    "sns.stripplot(x=pd.cut(df['Mean years of schooling'], bins=5), y='HDI Score', data=df, hue='HDI Tier', palette='viridis', jitter=0.25)\n",
    "plt.title('Mean Years of Schooling vs HDI Score')\n",
    "plt.xlabel('Mean Years of Schooling Bins')\n",
    "plt.ylabel('HDI Score')\n",
    "plt.xticks(rotation=45)\n",
    "os.makedirs('../static/images', exist_ok=True)\n",
    "plt.savefig('../static/images/schooling_vs_hdi.png', bbox_inches='tight', dpi=100)\n",
    "plt.show()"
   ]
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "## 4. Visualization: Life Expectancy vs HDI Score\n",
    "\n",
    "We create a scatter plot of Life Expectancy vs HDI Score."
   ]
  },
  {
   "cell_type": "code",
   "execution_count": None,
   "metadata": {},
   "outputs": [],
   "source": [
    "plt.figure(figsize=(10, 6))\n",
    "sns.scatterplot(x='Life expectancy', y='HDI Score', data=df, hue='HDI Tier', palette='coolwarm')\n",
    "plt.title('Life Expectancy vs HDI Score')\n",
    "plt.xlabel('Life Expectancy (Years)')\n",
    "plt.ylabel('HDI Score')\n",
    "plt.savefig('../static/images/life_expectancy_vs_hdi.png', bbox_inches='tight', dpi=100)\n",
    "plt.show()"
   ]
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "## 5. Visualization: Distribution Plots of Numeric Features"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": None,
   "metadata": {},
   "outputs": [],
   "source": [
    "fig, axes = plt.subplots(2, 2, figsize=(14, 10))\n",
    "sns.histplot(df['Life expectancy'], kde=True, ax=axes[0, 0], color='skyblue')\n",
    "axes[0, 0].set_title('Life Expectancy Distribution')\n",
    "\n",
    "sns.histplot(df['Mean years of schooling'], kde=True, ax=axes[0, 1], color='olive')\n",
    "axes[0, 1].set_title('Mean Years of Schooling Distribution')\n",
    "\n",
    "sns.histplot(df['Gross national income (GNI) per capita'], kde=True, ax=axes[1, 0], color='gold')\n",
    "axes[1, 0].set_title('GNI Per Capita Distribution')\n",
    "\n",
    "sns.histplot(df['Internet users'], kde=True, ax=axes[1, 1], color='teal')\n",
    "axes[1, 1].set_title('Internet Users Percentage Distribution')\n",
    "\n",
    "plt.tight_layout()\n",
    "plt.savefig('../static/images/distributions.png', bbox_inches='tight', dpi=100)\n",
    "plt.show()"
   ]
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "## 6. Visualization: Correlation Heatmap"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": None,
   "metadata": {},
   "outputs": [],
   "source": [
    "plt.figure(figsize=(8, 6))\n",
    "numeric_cols = ['Life expectancy', 'Mean years of schooling', 'Gross national income (GNI) per capita', 'Internet users', 'Expected years of schooling', 'HDI Score']\n",
    "corr = df[numeric_cols].corr()\n",
    "sns.heatmap(corr, annot=True, cmap='coolwarm', fmt='.3f', linewidths=0.5)\n",
    "plt.title('Correlation Heatmap of HDI Indicators')\n",
    "plt.savefig('../static/images/correlation_heatmap.png', bbox_inches='tight', dpi=100)\n",
    "plt.show()"
   ]
  }
 ],
 "metadata": {
  "language_info": {
   "name": "python"
  }
 },
 "nbformat": 4,
 "nbformat_minor": 2
}

# Write notebook file
os.makedirs("notebooks", exist_ok=True)
notebook_path = "notebooks/eda_and_training.ipynb"
with open(notebook_path, "w", encoding="utf-8") as f:
    json.dump(notebook, f, indent=2)

print(f"Jupyter Notebook generated at {notebook_path}")
